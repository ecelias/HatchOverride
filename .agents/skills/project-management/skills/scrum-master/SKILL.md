...
```bash
# Text report
python3 scripts/velocity_analyzer.py sprint_data.json --format text

# JSON output for downstream processing
python3 scripts/velocity_analyzer.py sprint_data.json --format json > analysis.json
```
...
```bash
python3 scripts/sprint_health_scorer.py sprint_data.json --format text
```
...
```bash
python3 scripts/retrospective_analyzer.py sprint_data.json --format text
```
...
1. Run velocity analysis: `python3 scripts/velocity_analyzer.py sprint_data.json --format text`
...
1. Run all three scripts before the session:
   ```bash
   python3 scripts/sprint_health_scorer.py sprint_data.json --format text > health.txt
   python3 scripts/retrospective_analyzer.py sprint_data.json --format text > retro.txt
   ```
...
```bash
python3 scripts/sprint_health_scorer.py team_data.json > health_assessment.txt
python3 scripts/retrospective_analyzer.py team_data.json > retro_insights.txt
```
...
- **Senior PM** (`../senior-pm/SKILL.md`) — Portfolio health context informs sprint priorities
